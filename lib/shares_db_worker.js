...
var SharesDbWorker = function(databaseName, worker)
{
...
SharesDbWorker.prototype.listenUp = function() {
  this.worker.on('account:removed', function(dbName) {
    if (dbName === this.userDatabaseName) {
...
// 
// 
SharesDbWorker.prototype.createUserSharesDatabase = function() {
  this.log('timeout done. Creating database %s ...', this.databaseName)
  this.couch.database(this.databaseName).create( function(error) {
    if (error) {
      this.log("Error creating datbase %s: %j", this.databaseName, error);
    ...
    this.log("Success! Created datbase %s", this.databaseName);
    this.startListeningToChanges();
...
SharesDbWorker.prototype.startListeningToChanges = function () {
  this.log('starting to listen for changes on %s', this.databaseName)
...
// 
SharesDbWorker.prototype._updateAccessSettings = function(databaseName, shareDoc) {
  this.log('_updateAccessSettings for ' + databaseName)
  ...
  this.when([
    ...
    this.log("ERROR in _updateAccessSettings: %j", error)
  });
}
SharesDbWorker.prototype._updateAccess = function(accessSetting) {
  ...
  if (accessSetting === undefined || accessSetting === false) {
    ...
    this.log("this.owner is %s", this.owner)
...
// 
// 
SharesDbWorker.prototype._sendSecurityUpdateRequest = function(databaseName, members, writers) {
  var options = {
    path   : '_security',
    method : 'PUT',
    json   : {
  ...
  this.couch.database(this.userDatabaseName).all({
    ...
    for (var i = 0; i < response.rows.length; i++) {
      ...
      if (row.doc.type === '$share' && row.doc.createdBy === this.owner) {
        ...
        this.log('row.doc.createdBy === %s (%s)', row.doc.createdBy, this.owner)
      }
    }
...
SharesDbWorker.prototype.dropShare = function(share_databaseName)
{
  ...
  this.couch.database(this.databaseName).all({
    ...
    var docsToDelete = [];
    if (error) {
      this.log("Error loading objects belonging to %s. %j", this.databaseName, error);
      return;
    }
...
// properties is set.
// 
SharesDbWorker.prototype._updateObjectForOtherShares = function(userDoc, sharedDoc, currentShareId) {
...
// 
// 
SharesDbWorker.prototype._merge = function(userDoc, sharedDoc, access) {
...
// 
// 
SharesDbWorker.prototype._mergeRemote = function(docId, sharedDoc, access) {
  var attributes;
  this.couch.database(this.databaseName).get(docId, function (error, otherSharedDoc) {
    ...
    this.couch.database(this.databaseName).query(options, function(error) {
      if (error) {
        this.log("ERROR: Couldn't PUT %s in %s: %j", otherSharedDoc._id, this.databaseName, error)
        return;
      } 

      this.log("SUCCESS PUT " + otherSharedDoc._id + " in " + this.databaseName)
    }.bind(this));
  }.bind(this))
}

...
var SharesDatabase = function(worker) {
  ...
  .otherwise( this.handleErrorWithMessage("bootstrap failed.") )
};
util.inherits(SharesDatabase, HoodieWorker);
...
SharesDatabase.prototype.handleBootstrapSucces = function (response) {
  ...
  if (! response.results) {
    return this.when.resolve(lastSeq)
  }
...
SharesDatabase.prototype.findAllObjectsBelongingTo = function( ownerHash ) {
  ...
  return this.when.resolve( objects )
}
...
SharesDatabase.prototype.removeObjects = function( objects ) {
  var save = this.promisify( this.database.save )
...
SharesDatabase.prototype.addShareObject = function(shareObject) {
  ...
  save(shareObject).otherwise( this.handleErrorWithMessage('could not share Object') )
};

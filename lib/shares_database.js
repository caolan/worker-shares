...
SharesDatabase.prototype.listenUp = function( last_seq ) {
  ...
  this.on('share:add',    this.addSubscription.bind(this) )
...
SharesDatabase.prototype.handleShareObjectChange = function( change ) 
{
  ...
  this.log('/%s/%s?rev=%s', this.name, change.id, change.doc._rev);
...
SharesDatabase.prototype.handleAccountRemovedError = function( error ) {
  this.worker.handleError(error, "could not cleanup objects after account:removed")
}
...
// 
// 
SharesDatabase.prototype.addSubscription = function(shareObject, originDatabaseName) {
  var save = this.worker.promisify( this.database, 'save' )
  ...
  if ( this.shareBelongsTo(shareObject, originDatabaseName) ) {
    subscriptionObject.source = originDatabaseName + "/shares"
...
SharesDatabase.prototype.removeSubscriptionObject = function(shareObject, originDatabaseName) {
  ...
  if (this.shareBelongsTo(shareObject, originDatabaseName)) {
    subscriptionId = this.makeSubscriptionId(originDatabaseName + "/shares", shareDatabaseName);
  } else {
    subscriptionId = this.makeSubscriptionId(shareDatabaseName, originDatabaseName + "/shares");
  }
  ...
  delete this.subscriptions[subscriptionObject._id];
...
SharesDatabase.prototype.makeSubscriptionId = function(databaseName1, databaseName2) {
  return '$subscription/' + [databaseName1, databaseName2].join(' => ');
};
...
SharesDatabase.prototype.subscriptionExists = function(shareObject, originDatabaseName) {
  var shareDatabaseName = this.getShareDatabaseName(shareObject)
  var subscriptionId = this.makeSubscriptionId(shareDatabaseName, originDatabaseName + "/shares")

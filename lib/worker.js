...
var SharesWorker = function(config) {
  ...
  console.log("setting up SharesWorker with:\n%s", JSON.stringify(config, '', '  '))
  this.setup(config)
...
SharesWorker.prototype.install = function() {
  return this.when.all([
    ...
    this.createUserSharesSkeletonDatabase(), 
...
// 
// 
SharesWorker.prototype.createUserSharesSkeletonDatabase = function() {
  var create = this.promisify( this.couch.database('skeleton/user/shares'), 'create' );

  this.log('creating skeleton/user/shares database …')
  return create()
  .otherwise( this.handleCreateDatabaseError('skeleton/user/shares').bind(this) )
  .then( this.handleCreateUserSharesSkeletonSuccess.bind(this) )
}
...
// 
// 
SharesWorker.prototype.handleCreateUserSharesSkeletonSuccess = function() {
  return this.when.all([
    this.createDesignDocsInUserSharesSkeleton(),
    this.createUserSharesSkeletonSecurity()
  ])
}
...
// 
// 
SharesWorker.prototype.createDesignDocsInUserSharesSkeleton = function() {
  this.log('creating design docs in skeleton/share …')

  var save = this.promisify( this.couch.database('skeleton/user/shares'), 'save')
  return save( shareFiltersDesignDoc._id, shareFiltersDesignDoc )
    .otherwise( this.handleErrorWithMessage("Could not save skeleton/user/shares/%s", shareFiltersDesignDoc._id) )
}
...
// 
// 
SharesWorker.prototype.createUserSharesSkeletonSecurity = function() {
  this.log('creating skeleton/user/shares/_security …')
  
  var query = this.promisify(this.couch.database("skeleton/user/shares"), 'query')
  var options = {
    path   : '_security',
    method : 'PUT',
    json   : userSharesSkeletonSecurityOptions
  };

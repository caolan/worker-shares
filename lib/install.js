...
var Install = function(couch, callback) {
  this.couch  = couch
  this.config = {}
  whenDone = function() {
  ...
  this.assureInstallation().then( whenDone ).fail( this._handleError.bind(this) )
};
...
Install.prototype = {
  ...
  readGlobalConfig: function() {
    ...
    this.couch.database('modules').get('global_config', function(error, object) {
      if (error) {
  ...
  setGlobalConfig: function(object) {
    this.config.app = object
  },

  setWorkerConfig: function(object) {
    // do not allow to set config.app, as it is reserved for

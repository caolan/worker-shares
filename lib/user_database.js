...
UserDatabase.prototype.handleSharedObjectChange = function(doc) {
  ...
  for(shareId in doc.$shares) {
    ...
    if ( doc.$shares[shareId] === false ) {
      ...
      this.log('triggering object:changed for %s', shareId)
    }
  }
  console.log("")
}
...
UserDatabase.prototype.handleObjectUnshare = function(shareId, object) {
  ...
  .otherwise( this.worker.handleErrorWithMessage("handleObjectUnshare: Could not update %s/%s?rev=", this.name, object._id, object._rev) )
};
...
UserDatabase.prototype.handleChangeFromSharesDatabase = function(change) {
  ...
  // ignore doc updates in my shares database that 
  // have been updated by me to avoid recursion
  if ( this.docChangedByMe(sharedObject)) {
    return
  }


  // "share/<shareId>/<type>/<objectId>" => "<shareId>"
  shareId   = sharedObject._id.match(/^share\/([^\/]+)/).pop()

  // "share/<shareId>/<type>/<objectId>" => "<type>/<objectId>"
  objectKey = sharedObject.match(/^share\/[^\/]+\/([^\/]+\/[^\/]+)$/).pop()

  loadObject(objectKey).then(
    this.handleLoadObjectSuccess( sharedObject, shareId, objectKey ).bind(this),
    this.handleLoadObjectError( sharedObject, shareId, objectKey ).bind(this)
  );
}
...
// 
// 
UserDatabase.prototype.handleLoadObjectSuccess = function( sharedObject, shareId, objectKey ) {
...
// 
// 
UserDatabase.prototype.handleLoadObjectError = function( sharedObject, shareId, objectKey ) {

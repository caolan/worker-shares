...
var UserSharesDatabase = function(databaseName, userDatabase)
{
  ...
  .otherwise( this.worker.handleErrorWithMessage("could not create %s", this.name) );
};
...
// 
// 
UserSharesDatabase.prototype.create = function() {
  var replicate = this.worker.promisify(this.couch, 'replicate')
  var options = {
    source        : "skeleton/user/shares",
    target        : this.name,
    create_target : true
  }
...
// 
//
UserSharesDatabase.prototype.handleCreateError = function(error) 
{ 
...
UserSharesDatabase.prototype.handleChange = function(change) {
  ...
  var shareId = change.id.match(/^share\/([^\/]+)/).pop()
...
UserSharesDatabase.prototype.getSharedObjectFromOriginal = function(shareId, originalObject) {
  var sharedObject
  var filter = originalObject.$shares && originalObject.$shares[shareId]

  // prepare our sharedObject
  sharedObject = {
    _id        : "share/" + shareId + "/" + originalObject._id
  }

  sharedObject = helpers.mergeProperties(sharedObject, originalObject, filter);
...
UserSharesDatabase.prototype.handleObjectUpdate = function( shareId, originalObject) {
  ...
  var sharedObject = this.getSharedObjectFromOriginal(shareId, originalObject);
...
UserSharesDatabase.prototype.updateSharedObject = function(sharedObject) {
  var getObject    = this.worker.promisify( this.database, 'get' )
  var updateObject = this.worker.promisify( this.database, 'save' )
  this.log('updating shared object: %s/%s with %j', this.name, sharedObject._id, sharedObject)

  this.database.get( sharedObject._id, function(error, objectFromSharesDatabase) {
    if (error) {
      if (error.error === 'not_found') {
        objectFromSharesDatabase = {}
      } else {
        throw new Error("updateSharedObject: error loading " + this.name + "/" + sharedObject._id)
      }
    }

    sharedObject._rev = objectFromSharesDatabase._rev
    updateObject(sharedObject)
    .otherwise(
      this.worker.handleErrorWithMessage("ERROR: Couldn't PUT %s/%s", this.name, sharedObject._id)
    );
  }.bind(this))
}
...
// 
// when an object has been unshared, 
...
UserSharesDatabase.prototype.handleObjectUnshare = function( shareId, object) {
  var sharedObject = this.getSharedObjectFromOriginal(shareId, object);
...
UserSharesDatabase.prototype.mergeAndUpdateObject = function(objectId, sharedDoc, access) {
  ...
  return function(otherSharedDoc) {
    ...
    // updatedBy value to prevent recursion.
    otherSharedDoc.updatedBy = "othershare"

    return this.updateSharedObject(otherSharedDoc)
  }
}
...
// 
// 
UserSharesDatabase.prototype.log = function(message) {

...
UserWorker.prototype._handleShareObjectUpdate = function(doc)
{
  ...
  if (this.shares[shareId]) {
    this.sharesWorker._updateReadAccessSettings()
  }
};
...
UserSharesWorker.prototype._create = function()
{
  couch.database(this.database_name).create( function(error) {
    ...
    this._updateReadAccessSettings(this.database_name, [this.owner]);
  }.bind(this));
};
...
// 
UserSharesWorker.prototype._updateReadAccessSettings = function(database_name, readAccess) {
  var readers = [],
      keys;
  ...
  switch (true) {
    ...
    case readAccess === true:
      // share is public read. Security Settings can remain to default
      this._sendSecurityUpdateRequest(database_name, readers);
      return;

    case readAccess === undefined:
    case readAccess === false:
      // share is private. Add owner as role
      readers.push(this.owner);
      this._sendSecurityUpdateRequest(database_name, readers);
      return;
    ...
    case Array.isArray(readAccess):
      ...
      // next, find roles of passed
      break;
  }
};

UserSharesWorker.prototype._sendSecurityUpdateRequest = function(database_name, readers) {
  var options = {
    path   : '_security',
    method : 'PUT',
    json   : {
...
UserSharesWorker.prototype.createShare = function(share_database_name, shareDoc)
{
  ...
  couch.database(share_database_name).create( function(error) {
    ...
    this._updateReadAccessSettings(this.database_name, shareDoc.access.read || shareDoc.access);
  });
};

...
UserWorker.prototype._handleSharedObjectUpdate = function(doc) {
  var shareId, sharedDoc, filter, attribute, currentRevNr, currentRevId, newRevisionId;

  if (doc._rev) {
    currentRevNr = 0;
  } else {
    currentRevNr = parseInt(doc._rev, 10);
    currentRevId = doc._rev.split(/-/).pop();
  }

  newRevisionId = this.__generateNewRevisionId();
  ...
  for(shareId in doc.$shares) {
    ...
    switch(filter) {
      ...
      case true: // share entire object
        ...
        for (var key in doc) {
          if (typeof sharedDoc[key] === 'undefined') {
    ...
    couch.database(this.shares_database_name).query({
      method: 'PUT', 
      path: shareDoc._id + "?new_edits=false", 
      body: shareDoc
    }); // TODO: handle error
  }
};

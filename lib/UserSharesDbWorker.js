...
UserSharesDbWorker.prototype = {
  ...
  _mergeRemote: function(docId, sharedDoc, access) {
    var attributes;
    this.couchConnection.database(this.userDatabaseName).get(docId, function (error, otherSharedDoc) {
      if (error) {
      ...
      this.couchConnection.database(this.userDatabaseName).query(options, function(error) {
        if (error) {
          this._log("ERROR: Couldn't PUT %s in %s: %j", otherSharedDoc._id, this.userDatabaseName, error)
          return;
        } 

        this._log("SUCCESS PUT " + otherSharedDoc._id + " in " + this.userDatabaseName)
      }.bind(this));
    }.bind(this))
  },

...
UserSharesDbWorker.prototype = {
  ...
  _updateAccessSettings: function(databaseName, shareDoc) {
    ...
    console.log(databaseName + ' readAccess')
    console.log(readAccess)
    console.log(databaseName + ' writeAccess')
    ...
    Q.when([
      ...
      console.log("FWFTFWTFWTFW!")
      console.log(JSON.stringify(promises[0].valueOf(), '', 2))
      var members = promises[0].valueOf(),
          writers = promises[1].valueOf();

      console.log(databaseName)
      console.log('%s members:', databaseName)
      console.log(JSON.stringify(members,'',2))
      console.log('%s writers:', databaseName)
      console.log(JSON.stringify(writers,'',2))
  ...
  _updateAccess: function(accessSetting) {
    ...
    this.couchConnection.database("_users").view('views/ownerByUsername', { keys: accessSetting}, function(error, results) {
      ...
      // TOOD: handle errors
      results.forEach( function(result) { list.push(result.value); });
      defer.resolve(list);
    }.bind(this));

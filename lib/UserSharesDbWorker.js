...
UserSharesDbWorker.prototype = {
  ...
  _startListeningToChanges: function () {
    ...
    this.feed = this.couchConnection.database(this.databaseName).changes({include_docs:true});
  ...
  createShare: function(share_databaseName, shareDoc) {
    ...
    this.couchConnection.replicate({
      ...
      this.couchConnection.database('_replicator').update("shares/start", replication_to_share, {filter: 'filters/share' });
      this.couchConnection.database('_replicator').update("shares/start", replication_to_user);
    }.bind(this));
  ...
  dropShare: function(share_databaseName)
  {
    ...
    this.couchConnection.database('_replicator').update("shares/stop", replication_to_share);
    this.couchConnection.database('_replicator').update("shares/stop", replication_to_user);
    ...
    this.couchConnection.database(this.databaseName).all({
      ...
      this.couchConnection.database(this.databaseName).save(docsToDelete);
    }.bind(this)); 
  },

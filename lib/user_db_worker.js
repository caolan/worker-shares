...
var UserDbWorker = function(databaseName, worker) {
  ...
  setTimeout( this.initSharesWorker.bind(this), 1000)
};
...
UserDbWorker.prototype.handleChangeError = function(error) {
  if (error && error.message.indexOf("Database deleted after change") !== -1) {
    this.log("Database %s has been dropped.", this.databaseName);
...
UserDbWorker.prototype.handleChange = function(change) {
  ...
  if (doc.type === "$share") {
    this.handleShareObjectUpdate(doc);
    return;
  }

  if (doc.$shares) {
    this.log('%s !== %s', doc.updatedBy, this.owner)
    ...
    this.handleSharedObjectUpdate(doc)
  }
}
...
// 
UserDbWorker.prototype.handleShareObjectUpdate = function(doc) {
  ...
  if (this.sharesWorker.shares[shareId] && this.sharesWorker.shares[shareId].createdBy !== this.owner) {
    ...
    if (this.sharesWorker.shares[shareId]) {
      ...
      if(this.accessSettingsChanged(this.sharesWorker.shares[shareId], doc)) {
...
// 
UserDbWorker.prototype.accessSettingsChanged = function(shareDoc1, shareDoc2) {
  return !this.readAccessSettingIsEqual(shareDoc1, shareDoc2) || !this.writeAccessSettingIsEqual(shareDoc1, shareDoc2);
}
UserDbWorker.prototype.readAccessSettingIsEqual = function(shareDoc1, shareDoc2) {
  var settings1 = shareDoc1.read || shareDoc1, 
      settings2 = shareDoc2.read || shareDoc2;

  this.accessSettingIsEqual(settings1, settings2);
}
UserDbWorker.prototype.writeAccessSettingIsEqual = function(shareDoc1, shareDoc2) {
  var settings1 = shareDoc1.write, 
      settings2 = shareDoc2.write;

  this.accessSettingIsEqual(settings1, settings2);
}
UserDbWorker.prototype.accessSettingIsEqual = function(settings1, settings2) {
...
// 
UserDbWorker.prototype.handleSharedObjectUpdate = function(doc) {
  var shareId, sharedObject, filter, attribute, options;

  this.log('handleSharedObjectUpdate')
  for(shareId in doc.$shares) {

    filter    = doc.$shares[shareId];
    this.log('shareId %s with filter %j', shareId, filter)
    sharedObject = this.prepareSharedDocUpdate(doc, shareId);
...
UserDbWorker.prototype.updateSharedObject = function(sharedObject) {
  ...
  this.couch.database(this.sharesDatabaseName).query(options, function(error) {
    if (error) {
      this.log("ERROR: Couldn't PUT %s in %s: %j", sharedObject._id, this.sharesDatabaseName, error)
      return;
    } 

    this.log("SUCCESS PUT " + sharedObject._id + " in " + this.sharesDatabaseName)
  }.bind(this));
}
...
// 
UserDbWorker.prototype.prepareSharedDocUpdate = function(originalDoc, shareId) {
...
// 
// 
UserDbWorker.prototype.generateNewRevisionId = function() {
  var timestamp, uuid;

...
UserDbWorker.prototype = {
  ...
  _handleShareObjectUpdate: function(doc) {
    ...
    if (this.sharesWorker.shares[shareId] && this.sharesWorker.shares[shareId].$createdBy !== this.owner) {
      console.log("")
      console.log("Subscription Update found: %s", shareId)
      console.log("unfortunately, subscriptions are not implemented yet")
      console.log("")
    } else {
      ...
      if (this.sharesWorker.shares[shareId]) {
        ...
        if(this._accessSettingsChanged(this.sharesWorker.shares[shareId], doc)) {
          ...
          console.log('nope. no security changes found for ' + shareId + '!')
        }
      }
    }
  },
